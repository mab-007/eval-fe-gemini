...
const StatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
  // Cast status to EvaluationStatus to ensure proper type checking
  const statusKey = status as EvaluationStatus;
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${styles[statusKey] || 'bg-gray-100 text-gray-700 border-gray-200'}`}>
      {icons[statusKey]}
      {status}
    </span>
  );
